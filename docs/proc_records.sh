...
## Папка, где находятся записи звонков. Например: "/var/spool/asterisk/monitor/". СЛЕШ на конце!
DIR_SOURCE="/var/spool/asterisk/monitor/"

## Конечная папка для файлов записей звонков, куда они будут перемещены. СЛЕШ на конце!
# Например: "/var/spool/asterisk/monitor/$Y/$M/$D/"
# Например: "/var/spool/asterisk/monitor/$Y/$YM/$YMD/"
DIR_DST="/var/spool/asterisk/monitor/$Y/$M/$D/"
...
# Например: Если 1 - поиск файлов только в папке /var/spool/asterisk/monitor/, 2 - поиск файлов в /var/spool/asterisk/monitor/ и в /var/spool/asterisk/monitor/[Любая_папка]/
# Можно указать, например: 9999. Это полезно, если настроено только конвертирование записей звонков, БЕЗ перемещения файлов.
# Тогда все файлы в /var/spool/asterisk/monitor/ и во всех вложенных папках будут преобразованы в MP3
DEPTH=1
